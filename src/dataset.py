...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_mask(self, img, index):
        ...
        if mask_type == 3:
            ...
            # Threshold mask: white (>=128) = vùng cần khôi phục, black (<128) = vùng giữ nguyên
            # Đảm bảo mask chỉ có 2 giá trị: 0 (black) hoặc 255 (white)
        ...
        if mask_type == 6:
            ...
            # Convert to grayscale if mask is RGB/RGBA
            if len(mask.shape) == 3:
                mask = rgb2gray(mask)
            # Ensure mask is 2D
            if len(mask.shape) > 2:
    ...
    def resize(self, img, height, width, centerCrop=True):
        ...
        # Use skimage.transform.resize instead of scipy.misc.imresize
        if len(img.shape) == 3:
            img = sk_resize(img, (height, width), preserve_range=True, anti_aliasing=True).astype(np.uint8)
        else:
            img = sk_resize(img, (height, width), preserve_range=True, anti_aliasing=True).astype(np.uint8)
